<script>
  ...
  onMount(async () => {
    ...
    console.log(descripcionDetallado)
    setTimeout(() => {
...
<Modal isOpen={true} scrollable fullscreen={"md"} size={"lg"}>
  ...
  <ModalBody>
    <main>
      <h5>
        Período: {periodo} valoracion:
        <span class:perdida={parseFloat(valoracion) < 3}
          >{valoracion != null ? valoracion : ""}</span
        >
      </h5>
      {#if descripcionDetallado.length > 0}
        <Descripcion {descripcionDetallado} />
      {:else}
        <div
