<script>
    ...
    async function tabChange(e) {
        ...
        [dataPeriodo] = await Promise.all([getData(periodo)]);

        let next =
            dataPeriodo[0].countNotas && dataPeriodo[0].countNotas.length === 4
                ? true
                : false;
    ...
    const getDataMap = async () => {
        ...
        markerLocations = locations.map((location) => {
            return [
                parseFloat(
                    location.coords.substring(0, location.coords.indexOf(","))
                ),
                parseFloat(
                    location.coords.substring(location.coords.indexOf(",") + 1)
                ),
                location.estudiante,
                location.nombres,
                location.nivel,
        ...
        markerLocations = markerLocations.map((location) => {
            return String([location[0].toFixed(4), location[1].toFixed(4),location[2],location[3],location[4],location[5]]);
        });
            ...
            <div>
                <h1>
                    <a
                        ...
                        href="#!"
                        on:click|preventDefault={openModalMap}
                        >{nivel}-{numero}</a
                    >
                </h1>
            </div>
